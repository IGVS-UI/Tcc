...
const isEntryPage = isLoginPage || isSignupPage;
...
onAuthStateChanged(auth, (user) => {
  if (user) {
    ...
    if (isEntryPage && !authFlowInProgress && !redirectTimer) {
    ...
    try {
      ...
      if (fullName) {
        try {
          ...
          console.warn("Conta criada, mas nao foi possivel salvar o nome:", profileErr);
        }
      }
...
if (btnGoogle) {
  btnGoogle.addEventListener("click", async () => {
    ...
    try {
      ...
      showSuccessModal(isSignupPage ? "Acesso concluido" : "Login concluido");
      queueRedirect("index.html");
    } catch (err) {
      authFlowInProgress = false;
...
if (btnGithub) {
  btnGithub.addEventListener("click", async () => {
    setStatus(isSignupPage ? "Conectando com GitHub..." : "Abrindo GitHub...");
    authFlowInProgress = true;
...
if (btnMicrosoft) {
  btnMicrosoft.addEventListener("click", () => {
    setStatus(
      isSignupPage
        ? "Cadastro com Microsoft ainda nao foi configurado."
        : "Login com Microsoft ainda nao foi configurado."
    );
  });
}
